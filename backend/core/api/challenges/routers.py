...
async def get_challenges_by_user(schema: ChallengesIdSchema,
    ...
    result = await session.execute(
        select(BaseUserModel.id_bu, UserModel.id_u, UserChallModel.id_ch).where(
            ChallengesModel.id_ch == schema["id_ch"]).join(UserChallModel,
                                                           ChallengesModel.id_ch == UserChallModel.id_ch).join(
            UserModel,
            UserChallModel.id_u == UserModel.id_u).join(BaseUserModel,
                                                        UserModel.base_user == BaseUserModel.id_bu).add_columns(
